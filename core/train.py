...
def get_args():
	...
	parser.add_argument('--rosta', type=str, required=False, default='TT',choices=["TT","FT","BT"], help="representations of semantic and tarnsition areas")
...
def save_last_checkpoint(args, model):
	...
	create_folder_if_not_exist(args.model_save_dir)
...
def main():
	...
	if args.rssn_denoise and args.bg_choice=='hd':
		...
		elif (not check_if_folder_exist(DATASET_PATHS_DICT['AM2K']['TRAIN']['FG_DENOISE_PATH'])) or (not check_if_folder_exist(DATASET_PATHS_DICT['BG20K']['TRAIN']['ORIGINAL_DENOISE_PATH'])):
	...
	if args.fg_generate=='closed_form':
		...
		elif (not check_if_folder_exist(DATASET_PATHS_DICT['AM2K']['TRAIN']['FG_PATH'])) or (not check_if_folder_exist(DATASET_PATHS_DICT['AM2K']['TRAIN']['BG_PATH'])):
